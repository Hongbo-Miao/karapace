...
from .backend import BaseBackupProducer, BaseBackupReader, BaseItemsBackupReader
...
from .v2 import AnonymizeAvroProducer, SchemaBackupV2Producer, SchemaBackupV2Reader
...
class BackupVersion(Enum):
    ...
    @property
    def producer(self) -> type[BaseBackupProducer]:
        if self is BackupVersion.V2:
            return SchemaBackupV2Producer
        if self is BackupVersion.ANONYMIZE_AVRO:
            return AnonymizeAvroProducer
        if self is BackupVersion.V1:
...
def _discover_reader_backend(fp: IO[str]) -> type[BaseBackupReader[IO[str]]]:
    try:
        ...
        # Seek back to start.
        fp.seek(0)
    # fixme: This is currently weird. Make it not weird.
    assert version is BackupVersion.V2
